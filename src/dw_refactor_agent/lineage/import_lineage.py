...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--lineage-file",
        default=None,
        help="可选的血缘 JSON 文件路径，默认使用 warehouses/{project}/artifacts/lineage/lineage_data.json",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
...
def insert_snapshot_row(
    ...
    *,
    ...
    cursor.execute(
        ...
        "column_lineage_count, indirect_lineage_count, table_lineage_count) "
    ...
    try:
        ...
        try:
            ...
            LOGGER.info(
                ...
                "job_datasets=%s indirect_edges=%s table_edges=%s batch_size=%s",
                len(rows.table_rows),
                len(rows.column_rows),
                len(rows.job_rows),
                len(rows.column_lineage_rows),
