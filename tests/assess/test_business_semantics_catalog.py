...
def test_write_initial_catalog_backfills_missing_split_files(
    tmp_path, monkeypatch
):
    project = _configure_catalog_project(monkeypatch, tmp_path)
    project_dir = tmp_path / project
    ...
    assert result["catalog"]["data_domains"][0]["id"] == "04"
...
def test_build_catalog_from_inspection_matches_unpadded_domain_id(
    tmp_path, monkeypatch
):
    project = _configure_catalog_project(monkeypatch, tmp_path)
    ...
    assert catalog["business_processes"][0]["data_domain"] == "04"
