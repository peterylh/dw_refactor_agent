...
""",
    ...
    for scenario_name, sql, expected_transient_tables in scenarios:
        facts = extract_task_table_facts(sql, "dws_orders.sql")

        assert facts["output_tables"] == {"dws_orders"}, scenario_name
        assert facts["transient_tables"] == expected_transient_tables, (
            scenario_name
        )
...
def test_extract_task_table_facts_repeated_and_case_variant_lifecycles():
    facts = extract_task_table_facts(
        """
DROP TABLE IF EXISTS shop_dm.tmp_orders_stage;
