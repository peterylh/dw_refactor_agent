...
```bash
python -m assess.llm.model_metadata_writer --project shop --mode catalog --dry-run
python -m assess.llm.model_metadata_writer --project shop --mode catalog
...
python -m assess.llm.model_metadata_writer --project finance_analytics --mode refresh --llm

# 从已确认 catalog 同步业务语义和基础模型元数据，不调用 LLM
python -m assess.llm.model_metadata_writer --project shop --mode refresh

# 忽略缓存，强制重新调用 DeepSeek
python -m assess.llm.model_metadata_writer --project shop --mode refresh --llm --no-cache

# LLM 返回校验失败时最多重试 2 次
python -m assess.llm.model_metadata_writer --project shop --mode refresh --llm --max-retries 2
```
