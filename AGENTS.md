...
必填字段: 所有表统一 CREATE TABLE 使用 `shop_dm.` 库名前缀。
...
将 `lineage/lineage_data.json` 导入到 Doris `lineage` 数据库。

### refresh_lineage_html.py

读取 `lineage/lineage_data.json`, 注入到 `lineage/lineage_job.html` 中刷新 HTML 可视化页面。
