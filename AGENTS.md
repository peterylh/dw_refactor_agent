...
active 表层级以模型 YAML 中的 `layer` 为唯一权威来源；v3 model 的
`classification` 被隔离时只有 `operational_layer`，语义层级必须按 unavailable 处理。
血缘与重构验证工具不再通过表名前缀兜底推断语义层级。
...
`generate` 不读取旧 models，也不会用旧执行配置兜底。它会从 task SQL 对目标表的 `TRUNCATE`、
目标表切片 DELETE、ETL 参数和 full-refresh 伴随任务确定性重建 `execution`。所有候选先在
内存中完成 typed recovery、section decision、依赖级联和发布校验。可恢复的单表语义问题只
隔离 `classification`、`business_semantics`、`entities`、`grain`、`metrics` 中受影响的
section；默认仍以 `publication.status=published_with_quarantine` 发布完整 v3 model 文件集。
被隔离 section 的正式字段必须缺席，只在 `governance.withheld_sections/reasons` 中记录状态；
未确认的业务过程/语义主题只进入结果 JSON/checkpoint 的 proposal audit，不写正式 catalog。
不带 `--llm` 的 generate 会发布只含 identity、`operational_layer`、description、execution 的
全 section quarantine v3 MID model。`--require-complete` 会把任一 quarantine 拒绝为
`not_published_incomplete`，且不改写正式 catalog/models。

执行契约、资产 manifest、未知 issue/schema 或事务校验等确定性错误仍为 `blocked`；系统性
巡检故障为 `not_published_inspection_failure`，两者都不发布。CLI 退出码为：成功或默认隔离
发布 `0`，hard block `1`，strict/巡检熔断 `2`，正式文件已发布但 checkpoint/report 收尾失败
`3`。`--dry-run` 始终零正式写入，并在 `would_publish_status` 中给出同一 gate 的预期结果。
generate 的资产和巡检角色来自 DDL/task 与内存候选，不扫描旧 model YAML；正式 confirmed
catalog 与完整 managed model set 共用 publication lock 和事务 journal 发布，普通异常回滚
整组文件。
...
- LLM 返回会按 DDL 字段名校验并生成 typed issue；安全修复后仍不可信的语义按 section 隔离
- refresh 遇到新的 semantic quarantine 时保留仍有效的现有 active section；已有 quarantine
  在当前结果通过且 confirmed catalog/合同闭合后才激活，并清理对应 governance reason
- 系统性巡检故障、未知 issue/schema 或 deterministic hard block 会阻止本轮项目级正式写入
...
**3000 行**时必须按职责拆分。若修改前文件已超过该阈值，本次涉及该文件的功能开发、
修复或重构必须同时完成拆分，不得继续扩大单文件规模。`warehouses/` 下的数仓资产及
生成工具、`benchmarks/`、`scripts/`、自动生成文件和第三方 vendored 代码不适用。
3000 行是强制拆分上限，不是建议目标；未达到该阈值时，仍应根据职责边界和维护成本
判断是否拆分。拆分必须按职责进行，不得仅为降低行数机械搬移代码。
