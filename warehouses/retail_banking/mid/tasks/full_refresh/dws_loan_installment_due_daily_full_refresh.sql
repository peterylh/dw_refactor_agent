...
TRUNCATE TABLE retail_banking_dm.dws_loan_installment_due_daily;
...
WHERE src.`duedate` IS NOT NULL
  AND (DATE(src.`duedate`) IS NULL OR (DATE(src.`duedate`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`duedate`) <= CAST(@etl_end_date AS DATE)))
GROUP BY
    DATE(src.`duedate`),
    src.`loan_id`;
