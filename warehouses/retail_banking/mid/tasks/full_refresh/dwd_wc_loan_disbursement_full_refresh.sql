...
WHERE (DATE(src.`actual_disburse_date`) IS NULL OR (DATE(src.`actual_disburse_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`actual_disburse_date`) <= CAST(@etl_end_date AS DATE)));
