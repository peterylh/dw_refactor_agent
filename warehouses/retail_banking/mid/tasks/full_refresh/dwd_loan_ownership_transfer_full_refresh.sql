...
WHERE (DATE(src.`effective_date_from`) IS NULL OR (DATE(src.`effective_date_from`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`effective_date_from`) <= CAST(@etl_end_date AS DATE)));
