...
WHERE (DATE(src.`proposed_transfer_date`) IS NULL OR (DATE(src.`proposed_transfer_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`proposed_transfer_date`) <= CAST(@etl_end_date AS DATE)));
