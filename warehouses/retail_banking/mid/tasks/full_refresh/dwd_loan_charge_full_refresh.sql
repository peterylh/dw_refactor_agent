...
TRUNCATE TABLE retail_banking_dm.dwd_loan_charge;
...
WHERE (DATE(src.`due_for_collection_as_of_date`) IS NULL OR (DATE(src.`due_for_collection_as_of_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`due_for_collection_as_of_date`) <= CAST(@etl_end_date AS DATE)));
