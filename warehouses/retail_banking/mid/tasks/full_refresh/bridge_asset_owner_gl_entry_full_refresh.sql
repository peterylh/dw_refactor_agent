...
WHERE (DATE(date_parent.`entry_date`) IS NULL OR (DATE(date_parent.`entry_date`) >= CAST(@etl_start_date AS DATE) AND DATE(date_parent.`entry_date`) <= CAST(@etl_end_date AS DATE)));
