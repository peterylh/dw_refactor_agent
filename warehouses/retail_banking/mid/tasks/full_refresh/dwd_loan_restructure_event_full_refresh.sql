...
WHERE (DATE(src.`submitted_on_date`) IS NULL OR (DATE(src.`submitted_on_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`submitted_on_date`) <= CAST(@etl_end_date AS DATE)));
