...
WHERE (DATE(src.`status_change_business_date`) IS NULL OR (DATE(src.`status_change_business_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`status_change_business_date`) <= CAST(@etl_end_date AS DATE)));
