...
WHERE (DATE(src.`aggregated_on_date`) IS NULL OR (DATE(src.`aggregated_on_date`) >= CAST(@etl_start_date AS DATE) AND DATE(src.`aggregated_on_date`) <= CAST(@etl_end_date AS DATE)));
