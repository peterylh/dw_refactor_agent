...
DELETE FROM retail_banking_dm.dwd_loan_ownership_transfer_detail
...
WHERE DATE(date_parent.`effective_date_from`) = CAST(@etl_date AS DATE)
   OR DATE(date_parent.`effective_date_from`) IS NULL;
