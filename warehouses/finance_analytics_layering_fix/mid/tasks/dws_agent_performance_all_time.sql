...
TRUNCATE TABLE finance_analytics_layering_fix_dm.dws_agent_performance_all_time;

INSERT INTO finance_analytics_layering_fix_dm.dws_agent_performance_all_time
WITH agent_list AS (
    SELECT DISTINCT
        agent_id
    FROM finance_analytics_layering_fix_dm.dwd_customer_interactions
...
agent_metrics AS (
    SELECT
        agent_id,
        CAST(MAX(interaction_date) AS DATE) AS as_of_date,
