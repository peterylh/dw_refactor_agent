-- ============================================================
-- 加工作业: ADS 促销投资回报分析表
-- 源表: dws_promotion_effect_daily, dim_promotion
-- 加工逻辑: 计算折扣成本 -> 计算ROI -> 填充默认值
-- 写入模式: 按 stat_date 分区, DELETE + INSERT 按日处理
-- ============================================================

SET @etl_date = COALESCE(@etl_date, CURDATE());
DELETE FROM shop_layering_fix_dm.ads_promotion_roi WHERE IF(@full_refresh = 1, 1=1, stat_date = CAST(@etl_date AS DATE));

INSERT INTO shop_layering_fix_dm.ads_promotion_roi
SELECT
    pe.promotion_id,
    pe.stat_date,
    pe.promotion_name,
    ...
    END AS roi,
    NOW() AS etl_time
FROM shop_layering_fix_dm.dws_promotion_effect_daily pe
WHERE IF(@full_refresh = 1, 1=1, pe.stat_date = CAST(@etl_date AS DATE));

UPDATE shop_layering_fix_dm.ads_promotion_roi
