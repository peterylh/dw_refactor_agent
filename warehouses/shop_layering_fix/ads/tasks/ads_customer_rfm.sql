...
-- Step 1: 删除当前统计日期的数据
DELETE FROM shop_layering_fix_dm.ads_customer_rfm WHERE IF(@full_refresh = 1, 1=1, stat_date = CAST(@etl_date AS DATE));

-- Step 2: 计算 RFM 指标 + NTILE 分项评分 + 综合评分 + 客户分层
INSERT INTO shop_layering_fix_dm.ads_customer_rfm
WITH rfm_base AS (
    SELECT
        customer_id,
    ...
    FROM shop_layering_fix_dm.dws_customer_order_summary
...
UPDATE shop_layering_fix_dm.ads_customer_rfm
