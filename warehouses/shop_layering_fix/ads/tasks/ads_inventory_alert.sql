-- ============================================================
-- 加工作业: ADS 库存预警分析表
-- 源表: dws_inventory_daily, dim_product, dim_store, dws_product_sales_daily
-- 加工逻辑: 关联维度 -> 计算日均销售速度 -> 预估库存支撑天数 -> 预警分级
-- 写入模式: 按 stat_date 分区, DELETE + INSERT 按日处理
-- ============================================================

SET @etl_date = COALESCE(@etl_date, CURDATE());
DELETE FROM shop_layering_fix_dm.ads_inventory_alert WHERE IF(@full_refresh = 1, 1=1, stat_date = CAST(@etl_date AS DATE));

INSERT INTO shop_layering_fix_dm.ads_inventory_alert
WITH sales_velocity AS (
    SELECT
        product_id,
        ROUND(AVG(sale_quantity), 2) AS daily_sales_velocity
    FROM shop_layering_fix_dm.dws_product_sales_daily
...
SELECT
    ...
    END AS alert_level,
    NOW() AS etl_time
FROM shop_layering_fix_dm.dws_inventory_daily inv
LEFT JOIN shop_layering_fix_dm.dim_product p
    ON inv.product_id = p.product_id
    AND p.snapshot_date = IF(@full_refresh = 1, DATE(inv.stat_date), CAST(@etl_date AS DATE))
LEFT JOIN shop_layering_fix_dm.dim_store s
...
UPDATE shop_layering_fix_dm.ads_inventory_alert
