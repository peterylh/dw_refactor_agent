...
-- 写入模式: 全量刷新, 非分区表按 stat_date 唯一键覆盖
-- ============================================================

-- Step 1: 清空目标表
TRUNCATE TABLE shop_layering_fix_dm.ads_sales_dashboard;

-- Step 2: 日度全店汇总 + LAG 计算环比增长率(从 DWD 层直接计算避免跨店客户重复)
INSERT INTO shop_layering_fix_dm.ads_sales_dashboard
WITH daily_base AS (
    SELECT
...
UPDATE shop_layering_fix_dm.ads_sales_dashboard
