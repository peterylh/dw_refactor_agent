...
DELETE FROM shop_layering_fix_dm.dws_category_sales_monthly
WHERE IF(@full_refresh = 1, 1=1, stat_month = DATE_FORMAT(@etl_date, '%Y-%m'));

-- Step 2: 按品类+月份汇总当月数据
INSERT INTO shop_layering_fix_dm.dws_category_sales_monthly
SELECT
    category_id,
...
-- Step 3: 销售数量为空时修正为 0
UPDATE shop_layering_fix_dm.dws_category_sales_monthly
SET sale_quantity = 0
