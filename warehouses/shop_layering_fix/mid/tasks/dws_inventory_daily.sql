...
DELETE FROM shop_layering_fix_dm.dws_inventory_daily WHERE IF(@full_refresh = 1, 1=1, stat_date = CAST(@etl_date AS DATE));

INSERT INTO shop_layering_fix_dm.dws_inventory_daily
SELECT
    product_id,
    store_id,
    ...
    NOW() AS etl_time
FROM shop_layering_fix_dm.dwd_inventory
