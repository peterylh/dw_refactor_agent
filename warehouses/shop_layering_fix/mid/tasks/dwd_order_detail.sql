...
-- Step 1: 删除当前日期的数据
DELETE FROM shop_layering_fix_dm.dwd_order_detail WHERE IF(@full_refresh = 1, 1=1, order_date = CAST(@etl_date AS DATE));

-- Step 2: 关联订单主表、明细表、商品表，构建订单明细宽表
INSERT INTO shop_layering_fix_dm.dwd_order_detail
SELECT
    o.order_id,
    oi.order_item_id,
    o.order_date,
    ...
    o.order_status,
    NOW() AS etl_time
FROM shop_layering_fix_dm.ods_order o
INNER JOIN shop_layering_fix_dm.ods_order_item oi ON o.order_id = oi.order_id
LEFT JOIN shop_layering_fix_dm.ods_product p ON oi.product_id = p.product_id
WHERE o.order_status = '已完成'
  AND IF(@full_refresh = 1, 1=1, o.order_date = CAST(@etl_date AS DATE));

-- Step 3: 商品维表关联缺失导致成本为空时，用售价×60%估算成本
UPDATE shop_layering_fix_dm.dwd_order_detail
