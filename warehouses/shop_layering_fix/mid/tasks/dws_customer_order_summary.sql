...
-- Step 1: 删除当前统计日期的数据
DELETE FROM shop_layering_fix_dm.dws_customer_order_summary WHERE IF(@full_refresh = 1, 1=1, stat_date = CAST(@etl_date AS DATE));

-- Step 2: 按客户+日期汇总订单指标
INSERT INTO shop_layering_fix_dm.dws_customer_order_summary
SELECT
    customer_id,
...
UPDATE shop_layering_fix_dm.dws_customer_order_summary
...
DELETE FROM shop_layering_fix_dm.dws_customer_order_summary
