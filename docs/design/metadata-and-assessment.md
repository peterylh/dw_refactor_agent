...
active 表层级只读取 Model 的 `layer`；classification 被隔离时只保留
`operational_layer`，语义层级为 unavailable。Catalog 提供可用 code 和治理关系，不反向
替代每张表自己的归属声明。

受管 Model 使用 v3 governance 表达不完整语义。`operational_layer` 是由资产位置和执行合同
确定的物理边界；`classification`、`business_semantics`、`entities`、`grain`、`metrics`
是可独立治理的语义 section。active section 的字段直接出现在 model 中；quarantined section
的字段必须完全缺席，并在 `governance.withheld_sections` 与 `reasons` 中记录原因。读取方必须
把 withheld 当作 unavailable，而不是空值或通过项。没有 withheld section 时不写
`governance`。
...
用于冷启动重建 Model。它从 frozen confirmed Catalog、DDL、Task SQL 和血缘生成新的模型
集合，不把现有 Model 当作推断先验。LLM 新发现的 code 只形成 proposal audit，不能在同一轮
进入 confirmed Catalog 或证明候选闭合。候选经过 section masking 与依赖固定点后，以完整
managed model set 发布；默认允许 `published_with_quarantine`，`--require-complete` 则要求
全部适用 section active。正式执行前会替换当前 Model，因此应先 dry-run 并通过 Git 审查变化。
...
- 业务 code 必须存在于已加载目录；
- DIM 不写指标分组；
- 可定位的语义问题按 section quarantine，且不得作为跨表证据；未知 schema/issue、确定性
  合同错误和系统性巡检故障仍阻止项目级发布。

Refresh 使用显式状态迁移：现有 active section 遇到新的 semantic quarantine 时保留已确认
内容，quarantined section 只有在当前结果通过且 confirmed Catalog/合同闭合后才激活并删除
stale reason。`--require-complete` 只改变 publication gate，不改变 decision 或 cache 语义。

Generate、LLM refresh 和独立 Catalog writer 共用项目 publication lock。正式内容先 staged
并 fsync，再由 journal 驱动安装、验证和清理；普通异常回滚整组文件。若文件已发布但
checkpoint/report 收尾失败，结果必须单独报告 `formal_files_state=published` 和
`finalization_status=failed`，不能伪装成未发布。
...
- `generation_candidate_resolver.py`：typed issue 到 section decision、字段 masking 与依赖固定点；
- `publication_transitions.py`：generate/refresh gate、状态迁移和 strict completeness 语义；
...
- 直接决定规则是否通过；
- 在无人工审查时批量接受不可逆元数据变化。
- 让 proposal、quarantined payload、旧 cache/checkpoint effective view 进入正式 Catalog 或
  冒充 active 跨表证据。
