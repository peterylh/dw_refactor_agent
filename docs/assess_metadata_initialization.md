...
复检使用不同上下文哈希，可分别恢复。有效结构化结果即使因非 retryable 语义问题产生
quarantine，也可以复用原始 parsed payload；恢复后会重新执行当前 policy，所以不会把旧的
section decision 当作可信结论。transport/parser/低置信度、内部错误、传播未收敛、缺失、损坏
或输入已经变化的结果仍会重新巡检。因此项目级发布失败后，保持输入不变重跑时只消耗失败表
或失效上下文的 API 调用。
...
检查点标记为 `finalization_failed`。结果中的 `inspection_reuse` 以及 checkpoint 报告会记录
accepted/quarantine 命中、命中率、失效原因和触发重裁决的 policy version。开启默认进度输出
时会显示检查点命中；`--quiet` 可关闭。
