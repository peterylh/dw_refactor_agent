...
dw-refactor cleanup delete --project shop --database shop_dm_qa \
  --include-legacy --yes
```
...
legacy/invalid 槽不进入普通 ID 或时间删除。它们只能通过精确数据库名加显式
`--include-legacy` 或 `--include-invalid` 预览并配合 `--yes` 清理；数据库必须是项目
配置中的 pool member。该强制选项仍不得操作生产库、lineage 库或未配置数据库。
...
- legacy/invalid 必须显式强制且只能针对已配置 pool member。
